...
class MyImageDataGenerator(ImageDataGenerator):
    ...
    def random_crop(self, original_img):
        ...
        if original_img.shape[0] < self.random_crop_size[0]
        or original_img.shape[1] < self.random_crop_size[1]:
    ...
    def flow_from_directory(
        ...
        batches = super().flow_from_directory(
            directory, target_size, color_mode, classes, class_mode, batch_size, shuffle, seed, save_to_dir, save_prefix, save_format, follow_links, subset, interpolation
        )
        # 拡張処理
        while True:
